...
Everything is implemented and covered by tests (156 tests green): the `workspaceEnvrc` provider core, the reversible Bash adapter, the persistent-terminal adapter, the workspace MCP adapter (plan §11 fully implemented: source, deterministic unit tests, a real MCP SDK fixture, and real WorkspaceTree hot-reload plus native direnv composition), the integration row, and three real-composition test paths (a real `direnv` allow/deny/content-change state machine, a real Cordis Loader composing the built `dist` provider/integration rows, and a full lifecycle of the real MCP SDK stdio fixture + native direnv + top-level config hot reload). The implementation plan [`implementation-plan.md`](./implementation-plan.md) marks both §9 and §11 **implemented and published**. This README describes the current implementation facts and no longer narrates the work in historical blocks.
...
- **`enableWorkspaceMcp: false`**: the MCP adapter stays installed but is permanently transparent — workspace stdio rows start directly (**never through direnv**, even when the `.envrc` is blocked), and the explicit config env reaches the child verbatim (no shim runs, so config `DSH_*` entries are not deleted either).
...
- **Sandbox boundary**: the Bash/terminal direnv chain runs inside the DSH executor's or terminal's confinement, so that sandbox must be able to read the native allow database; an `XDG_DATA_HOME` the sandbox masks (for example `/tmp` under the local bwrap profile's tmpfs) makes an externally allowed `.envrc` appear blocked. The current overlay spawns workspace MCP through the MCP SDK directly and exposes no sandbox/confine seam, so the MCP process and its `.envrc` evaluation do not pass through the DSH sandbox; this is an explicit boundary, and the terminal confinement guarantee must not be extrapolated to MCP.
...
- **No watcher / no auto restart**: this bundle watches no files (including `.envrc` — content changes are rejected by the native hash on the next execution). Changing only `.envrc` never touches a running process: an MCP process freezes its environment at spawn and keeps its pid, and running background jobs and terminals are not restarted either; a content change only makes **future** spawns fail through native direnv until the user re-allows outside DSH. Saving/touching the workspace's top-level `.dsh/cordis.yml` goes through the existing WorkspaceTree hot reload — the old MCP process exits and a fresh one starts through direnv; when the fresh start is blocked, the workspace's tools/mask are temporarily absent per the overlay's existing failure semantics while the Agent and the workspace lease stay alive, and the next top-level config event retries; after the user re-allows, saving the config again yields a fresh pid/tools/mask. The global row is never disturbed by workspace reloads (process and view stay live throughout); the final release/dispose exits every MCP process with zero residue.
...
The full suite of 156 tests is green, and the tests never read or write the real user's direnv authorization state: `tests/direnv-native.spec.ts` drives the full allow/deny/content-change state machine and the deferred terminal wrapper through the real `direnv`, with all authorization state confined to repo-internal isolated `XDG_DATA_HOME`/`XDG_CONFIG_HOME`/`XDG_CACHE_HOME`/`HOME` (under `.artifacts/`, gitignored); the shim script and the wrapped command run through real child processes with explicit isolated environments (no `process.env` mutation); `tests/mcp-adapter.spec.ts` deterministically covers the MCP adapter's scope classification/passthrough/lifecycle and executes the wrapped MCP argv through a repo-internal fake direnv child; `tests/mcp-live-direnv.spec.ts` (fixture: `tests/fixtures/mcp/fixture-server.mjs`, a real `@modelcontextprotocol/sdk` `Server`/`StdioServerTransport`) runs against the real overlay registry (real chokidar hot reload), the real `WorkspaceMcpManager`/`workspace-client`, the real provider/integration, and native direnv under repo-internal isolated XDG/HOME, proving end to end: an allowed `.envrc` reaching the MCP child with native precedence and credential-shaped visibility, the empty managed snapshot (every `DSH_*` name from config/.envrc/ambient absent), the manager-resolved workspace cwd, a global same-serverName row byte-for-byte unwrapped and untouched by workspace reloads, no `.envrc` watcher (the v1 process frozen on `.envrc`-only edits), a blocked reload (old process exits, workspace tools/mask removed, lease and scope stay alive, canary-free diagnostics), re-allow recovery (fresh pid, v2 environment, restored mask), final release/dispose with zero process residue (marker reconciliation), and `enableWorkspaceMcp: false` starting without direnv even with a blocked `.envrc`; the background path is verified through the real AgentRegistry + ToolRuntime + tool-bash + jobs provider; the terminal path through the real TerminalSessionService + terminal-bash + SandboxPolicyService; `tests/loader-composition.spec.ts` composes the built `dist` provider/integration rows with real DSH services and overlay dependencies through a real Cordis Loader reading a test `cordis.yml`; the built-entry smoke (`.artifacts/smoke-built.mjs`) resolves the root/core/adapter/integration exports of `dist` against the installed DSH rc.6 packages and runs the real production preflight.
