...
The current overlay MCP transport delegates directly to the MCP SDK's stdio spawn and exposes no sandbox/confine seam. Consequently the workspace MCP process and its direnv/`.envrc` evaluation run with the MCP transport's host process ownership, not inside DSH's Bash/terminal sandbox. This extension documents that boundary rather than claiming a confinement guarantee the underlying transport cannot provide.
