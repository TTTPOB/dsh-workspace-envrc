...
writeFileSync(join(destination, 'release-notes.md'), [
  ...
  '',
  '```sh',
  `dsh plugin --profile web add ${pkg.devDependencies['dsh-workspace-overlay']}`,
  `dsh plugin --profile web add ${repository}/releases/download/${tag}/${filename}`,
  'dsh --profile web --dump-config',
  '```',
  '',
  'Requires host-installed direnv. Keep dsh-workspace-overlay before dsh-workspace-envrc in dsh.profile.bundles. Restart the Host after installation.',
  '',
].join('\n'))
